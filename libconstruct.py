...
def mechanism(context, role):
	"""
	Get the mechanism from the libconstruct directory for the given role.

	This extracts the data from the XML files generated by &.bin.configure.
	"""
	global merge, libconstruct_transforms

	doc, core_data = libconstruct_transforms(context, 'core')
	doc, role_data = libconstruct_transforms(context, role)
...
def initialize(selection:str, context:str, role:str, module:libdev.Sources):
	...
	core_data = mechanism(selection, role)
